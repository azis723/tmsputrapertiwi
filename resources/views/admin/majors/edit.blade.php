...
@section('title', 'Edit Jurusan & Geofencing')
@section('page-title', 'Edit Jurusan: ' . $major->name)
@section('page-subtitle', 'Sesuaikan titik koordinat dan radius toleransi geofencing presensi')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <form method="POST" action="{{ route('admin.majors.update', $major) }}" class="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $major->name) }}" required
            ...
            <div>
                ...
                <input type="text" name="code" id="code" value="{{ old('code', $major->code) }}" required
        ...
        <div class="pt-4 border-t border-slate-800 space-y-4">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                    <h3 class="text-sm font-bold text-white uppercase tracking-wider">Visual Map Editor Geofencing</h3>
                    <p class="text-xs text-slate-400 mt-0.5">Geser marker pin di bawah untuk memindahkan titik pusat geofence jurusan</p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="number" step="0.00000001" name="latitude" id="latitude" value="{{ old('latitude', $major->latitude) }}" required readonly
                ...
                <div>
                    ...
                    <input type="number" step="0.00000001" name="longitude" id="longitude" value="{{ old('longitude', $major->longitude) }}" required readonly
                           class="w-full px-4 py-2.5 rounded-xl bg-slate-950/80 border border-slate-800 text-slate-300 text-sm font-mono focus:outline-none">
                ...
                <div>
                    ...
                    <input type="number" name="radius_meters" id="radius_meters" value="{{ old('radius_meters', $major->radius_meters) }}" min="10" max="5000" required
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const cabeLat = -6.3472;
        const cabeLng = 106.7645;
        const latInput = document.getElementById('latitude');
        const lngInput = document.getElementById('longitude');
        const radiusInput = document.getElementById('radius_meters');

        const initialLat = parseFloat(latInput.value) || cabeLat;
        const initialLng = parseFloat(lngInput.value) || cabeLng;
        ...
        document.getElementById('btn-reset-cabe').addEventListener('click', function () {
            map.setView([cabeLat, cabeLng], 16);
            marker.setLatLng([cabeLat, cabeLng]);
            circle.setLatLng([cabeLat, cabeLng]);
            latInput.value = cabeLat.toFixed(8);
            lngInput.value = cabeLng.toFixed(8);
        });
    });
</script>
@endpush
