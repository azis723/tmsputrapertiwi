...
@section('page-subtitle', 'Pencatatan data tenaga pendidik untuk referensi mapel dan wali kelas')
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <form method="POST" action="{{ route('admin.teachers.store') }}" class="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Contoh: Budi Santoso, S.Kom."
            ...
            <div>
                ...
                <input type="text" name="nip" id="nip" value="{{ old('nip') }}" placeholder="19850101..."
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <input type="email" name="email" id="email" value="{{ old('email') }}" required placeholder="guru@putrapertiwi.sch.id"
                       class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 focus:ring-2 focus:ring-indigo-500 focus:outline-none text-sm">
        ...
        <div>
            ...
            <textarea name="address" id="address" rows="2" placeholder="Alamat lengkap guru..."
                      class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 focus:ring-2 focus:ring-indigo-500 focus:outline-none text-sm">{{ old('address') }}</textarea>
        ...
        <div>
            ...
            <select name="status" id="status" class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white focus:ring-2 focus:ring-indigo-500 focus:outline-none text-sm">
                <option value="active" {{ old('status', 'active') === 'active' ? 'selected' : '' }}>Aktif Mengajar</option>
