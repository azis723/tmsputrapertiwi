...
@section('page-title', 'Edit Kelas: ' . $class->name)
@section('page-subtitle', 'Perbarui jurusan, tahun akademik, atau wali kelas')

@section('content')
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.classes.index') }}" class="text-xs text-slate-400 hover:text-white flex items-center gap-1.5 transition">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            <span>Kembali ke Data Kelas</span>
        </a>
    </div>

    <form method="POST" action="{{ route('admin.classes.update', $class) }}" class="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="name" class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Nama Kelas</label>
        ...
        <div>
            <label for="major_id" class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Kompetensi Keahlian / Jurusan</label>
            <select name="major_id" id="major_id" required class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white focus:ring-2 focus:ring-indigo-500 focus:outline-none text-sm">
                <option value="">-- Pilih Jurusan --</option>
                @foreach ($majors as $m)
                    <option value="{{ $m->id }}" {{ old('major_id', $class->major_id) == $m->id ? 'selected' : '' }}>
                        {{ $m->name }} ({{ $m->code }})
                    </option>
                @endforeach
            </select>
            @error('major_id') <p class="text-rose-400 text-xs mt-1.5">{{ $message }}</p> @enderror
        </div>
        ...
        <div>
            <label for="teacher_id" class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Wali Kelas (Referensi Guru)</label>
            <select name="teacher_id" id="teacher_id" class="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-white focus:ring-2 focus:ring-indigo-500 focus:outline-none text-sm">
                <option value="">-- Belum Ditugaskan / Pilih Wali Kelas --</option>
        ...
        <div class="pt-4 border-t border-slate-800 flex justify-end gap-3">
            <a href="{{ route('admin.classes.index') }}" class="px-5 py-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm font-medium transition">
                Batal
            </a>
