...
@section('page-title', 'Edit Siswa: ' . $student->name)
@section('page-subtitle', 'Perbarui data siswa dan kelola kredensial akun')
...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        ...
        @if ($student->hasEnrolledFace())
            <form method="POST" action="{{ route('admin.students.reset-face', $student) }}" onsubmit="return confirm('Apakah Anda yakin ingin mereset biometrik wajah siswa ini?')">
                @csrf
                <button type="submit" class="px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/30 hover:bg-amber-500/20 text-amber-400 text-xs font-semibold transition">
                    Reset Biometrik Wajah Siswa
                </button>
            </form>
        @endif
    </div>

    <form method="POST" action="{{ route('admin.students.update', $student) }}" class="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
        @csrf
        @method('PUT')
