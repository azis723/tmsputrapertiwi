...
@section('page-subtitle', 'Setup mata pelajaran baru dan pilih guru pengampu')
...
<div class="max-w-xl mx-auto space-y-6">
    ...
    <form method="POST" action="{{ route('admin.subjects.store') }}" class="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
        @csrf

        <div>
            <label for="code" class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">Kode Mapel</label>
            <input type="text" name="code" id="code" value="{{ old('code') }}" required placeholder="Contoh: RPL-01"
        ...
        <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Contoh: Pemrograman Web & Perangkat Bergerak"
